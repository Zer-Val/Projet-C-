...
 with a pure virtual method double payo(double), payo() represents the payo function
...
factor 1 + R > 0 at each time step (starting at 1 at step 0).
The model admits no arbitrage iif D < R < U.
4
In the CRR model the price H(n, i) at time step n and node i of a European option with
expiry date N and payo h (S (N)) can be computed using the CRR procedure, which proceeds
